...

{
  ...
  assert.equal(backup.namespaces.friendship_trade_state.present, true);
  assert.equal(backup.namespaces.search_templates.data.templates[0].name, "Cleanup");
  assert.equal(backup.namespaces.friendship_trade_state.data.friends[0].wishes[0], "Mewtwo");
  ...
  assert(validated.preview.added.includes("friendship_trade_state"));
}
