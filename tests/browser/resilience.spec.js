...
async function expectFailClosed(page) {
  ...
  await expect(page.locator("#result-count")).toHaveText("Collection data could not be loaded", { timeout: 20_000 });
...
function desktopChromiumOnly(testInfo) {
  test.skip(
    ...
    "Critical resilience scenarios intentionally run once in pinned desktop Chromium.",
  );
}
