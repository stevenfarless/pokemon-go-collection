...
class PrivacyProfilesTests(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (data / "latest-export.csv").open("w", encoding="utf-8", newline="") as handle:
                writer = csv.DictWriter(
                    handle,
                    fieldnames=["Index", "Name", "Pokemon Number", "CP", "Scan Date", "Catch Date"],
                )
                writer.writeheader()
                writer.writerow({
                    "Index": "7",
                    ...
                    "Catch Date": "2026-08-18",
                })
            (root / "index.html").write_text(
                '<html><head><meta name="description" content="Browse collection and add Friend Code 2252 2231 2780."></head>'
                '<body><div class="trainer-contact">Friend Code: 2252 2231 2780</div>Fuddledumpy’s Pokémon GO Collection</body></html>',
                encoding="utf-8",
            )
            ...
            self.assertNotIn("2252 2231 2780", html)
            self.assertNotIn("add Friend Code", html)
            self.assertFalse(audit["friend_code_public"])
            self.assertEqual(len(audit["published_export_sha256"]), 64)
    ...
    def test_full_public_keeps_friend_code_metadata(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            data = root / "data"
            data.mkdir()
            (data / "latest-export.csv").write_text("Name,Pokemon Number,CP\nPikachu,25,500\n", encoding="utf-8")
            html = '<html><head><meta name="description" content="Browse collection and add Friend Code 2252 2231 2780."></head><body>Fuddledumpy’s Pokémon GO Collection</body></html>'
            (root / "index.html").write_text(html, encoding="utf-8")
            privacy_profiles.prepare_privacy(root, {"POKEMON_GO_PRIVACY_PROFILE": "full-public"})
            result = (root / "index.html").read_text(encoding="utf-8")
            self.assertIn("add Friend Code 2252 2231 2780", result)
