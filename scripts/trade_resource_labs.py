...
ITEM_BAG_CATEGORIES = [
    "poke_ball", "great_ball", "ultra_ball", "razz_berry", "nanab_berry", "pinap_berry",
    "golden_razz", "silver_pinap", "potion", "super_potion", "hyper_potion", "max_potion",
    "revive", "max_revive", "fast_tm", "charged_tm", "elite_fast_tm", "elite_charged_tm",
    "evolution_item", "raid_pass", "premium_battle_pass", "remote_raid_pass", "incense", "lure",
    "lucky_egg", "star_piece", "incubator", "bottle_cap",
]
...
def build_trade_contract(manifest: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "inputs": {
            ...
            "player_b": {"source": "browser-selected Poke Genie CSV", "preflight_contract": "data/preflight-contract.json", "identity": "ephemeral guest-row-N only", "persistence": "none"},
            "knowledge": "data/knowledge/species-index.json",
        },
        "privacy": {"guest_bytes_leave_browser": False, "guest_rows_persisted": False, "guest_rows_cached": False, "guest_rows_published": False, "clear_session_discards_guest": True},
        "matching": {
            "possible_mutual_wins_first": True, "surplus_threshold": 2, "exact_player_a_records": True,
            ...
            "special_trade": "May be flagged only from explicit supported species/category evidence; eligibility and cost still require in-game confirmation.",
            "lucky": "No Lucky outcome is guaranteed.", "stardust": "No exact trade Stardust cost is calculated without complete current friendship/history inputs.",
            "filters": {"scope": "current in-memory guest comparison only", "goal": "case-insensitive species/form text within a proposed pair", "family": "versioned knowledge family id", "rarity": "explicit source-tag categories only; unsupported classifications remain unknown", "manual_exclusions_persisted": False},
        },
        "handoff": {"action_packs": "action-packs.html", "player_a": "narrow exact-owned locator when representable", "player_b": "species/form review helper; guest rows have no canonical record ID"},
    }


def build_resource_contract(manifest: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": RESOURCE_VERSION, "lab_version": LAB_VERSION, "build_id": manifest["build_id"], "title": "Trainer Resource Vault",
        "storage": {"key": "pokemon-go-collection:resource-vault:v1", "schema_version": 1, "local_only": True, "unified_backup": True, "bounded_history": 12},
        "resources": RESOURCE_TYPES,
        "semantics": {"missing_balance": "unknown, never zero", "reserve": "amount intentionally unavailable to ordinary plans", "commitment": "named local reservation that participates in conflict detection", "plan": "what-if cost only; never claims the in-game balance changed", "allocation": "deterministic priority/order evaluation across selected plans"},
        "safety": {"infer_balances_from_collection": False, "mutate_canonical_collection": False, "double_spend_silent": False, "scarce_resources": ["rare_candy_xl", "elite_fast_tm", "elite_charged_tm", "silver_bottle_cap", "gold_bottle_cap"], "scarce_resource_warning_required": True},
...
def build_item_bag_contract(manifest: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema_version": ITEM_BAG_VERSION, "lab_version": LAB_VERSION, "build_id": manifest["build_id"], "title": "Item Bag Planner",
        "storage": {"key": "pokemon-go-collection:item-bag:v1", "schema_version": 1, "local_only": True, "unified_backup": True},
        "inputs": {"required": ["bag capacity", "play-style profile"], "optional": ["item counts", "reserves", "manual protections", "expiration timestamps", "custom target ranges"], "missing_count": "unknown"},
        "profiles": ["balanced", "catching", "raid", "pvp", "rocket", "max", "rural", "custom"],
        "categories": ITEM_BAG_CATEGORIES,
        "cleanup": {"scenario_slots_default": 50, "never_cross_profile_target_minimum": True, "user_reserve_overrides_target": True, "rare_or_protected_excluded": True, "action": "review-only; no account or in-game mutation"},
        "events": {"source": "data/event-calendar.json", "requires_actionable_at_build": True, "requires_fresh_source_state": True, "stale_or_missing": "no event-specific adjustment"},
        "safety": {"rare_time_limited_auto_discard": False, "missing_as_zero": False, "canonical_collection_mutation": False, "account_access": False},
    }
...
def schemas() -> dict[str, dict[str, Any]]:
    string = {"type": "string"}; build = {"type": "string", "pattern": "^[0-9a-f]{12}$"}
    return {
        "trade-resource-labs-index.schema.json": _schema("trade-resource-labs-index", ["schema_version", "build_id", "labs"], {"schema_version": string, "build_id": build, "labs": {"type": "object"}}),
        "trade-matcher-contract.schema.json": _schema("trade-matcher-contract", ["schema_version", "build_id", "inputs", "privacy", "matching", "handoff"], {"schema_version": string, "build_id": build, "inputs": {"type": "object"}, "privacy": {"type": "object"}, "matching": {"type": "object"}, "handoff": {"type": "object"}}),
        "resource-vault-contract.schema.json": _schema("resource-vault-contract", ["schema_version", "build_id", "storage", "resources", "semantics", "safety"], {"schema_version": string, "build_id": build, "storage": {"type": "object"}, "resources": {"type": "array"}, "semantics": {"type": "object"}, "safety": {"type": "object"}}),
        "item-bag-contract.schema.json": _schema("item-bag-contract", ["schema_version", "build_id", "storage", "inputs", "profiles", "categories", "cleanup", "events", "safety"], {"schema_version": string, "build_id": build, "storage": {"type": "object"}, "inputs": {"type": "object"}, "profiles": {"type": "array"}, "categories": {"type": "array"}, "cleanup": {"type": "object"}, "events": {"type": "object"}, "safety": {"type": "object"}}),
    }


def _register_contracts() -> None:
    manifest_registry._SCHEMA_MAP.update({"data/trade-resource-labs/index.json": "data/trade-resource-labs-index.schema.json", "data/trade-matcher-contract.json": "data/trade-matcher-contract.schema.json", "data/resource-vault-contract.json": "data/resource-vault-contract.schema.json", "data/item-bag-contract.json": "data/item-bag-contract.schema.json"})
    manifest_registry._STABLE_NAMES.update({
        "data/trade-resource-labs/index.json": "trade_resource_labs_index", "data/trade-matcher-contract.json": "trade_matcher_contract", "data/resource-vault-contract.json": "resource_vault_contract", "data/item-bag-contract.json": "item_bag_contract",
        "data/trade-resource-labs-index.schema.json": "trade_resource_labs_index_schema", "data/trade-matcher-contract.schema.json": "trade_matcher_contract_schema", "data/resource-vault-contract.schema.json": "resource_vault_contract_schema", "data/item-bag-contract.schema.json": "item_bag_contract_schema",
    })


def _page(output_dir: Path, filename: str, title: str, mount_id: str, description: str, body: str) -> None:
    html = f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{title}</title><link rel="stylesheet" href="assets/trade-resource-labs.css" data-trade-resource-style></head><body><main class="trade-resource-page"><header class="trade-resource-header"><p><a href="tools.html">Tools</a> · <a href="index.html">Collection</a> · <a href="trade-matcher.html">Trade Matcher</a> · <a href="resource-vault.html">Resource Vault</a> · <a href="item-bag.html">Item Bag</a> · <a href="action-packs.html">Action Packs</a></p><h1>{title}</h1><p>{description}</p></header>{body}<div id="{mount_id}" aria-live="polite"></div></main><script defer src="assets/trade-resource-labs.js" data-trade-resource-script></script></body></html>'''
...
def _install_tools_links(output_dir: Path) -> None:
    ...
    if 'id="trade-resource-labs"' not in source:
        ...
        marker = "  </main>"
        if marker not in source: raise ValueError("Generated tools page is missing its main closing tag")
        source = source.replace(marker, block + marker, 1)
    if 'data-item-bag-tools' not in source:
        body_marker = "</body>"
        if body_marker not in source: raise ValueError("Generated tools page is missing its body closing tag")
        source = source.replace(body_marker, '  <script defer src="assets/item-bag-planner.js" data-item-bag-tools></script>\n  <script defer src="assets/trade-resource-labs.js" data-trade-resource-tools></script>\n</body>', 1)
...
def publish(repository_root: Path, output_dir: Path, manifest: Mapping[str, Any]) -> dict[str, Any]:
    del repository_root
    _register_contracts()
    trade, vault, item_bag = build_trade_contract(manifest), build_resource_contract(manifest), build_item_bag_contract(manifest)
    _write(output_dir / "data" / "trade-matcher-contract.json", trade); _write(output_dir / "data" / "resource-vault-contract.json", vault); _write(output_dir / "data" / "item-bag-contract.json", item_bag)
    index = {"schema_version": LAB_VERSION, "build_id": manifest["build_id"], "labs": {
    ...
    for filename, contract in schemas().items(): Draft202012Validator.check_schema(contract); _write(output_dir / "data" / filename, contract)
    ...
    _page(output_dir, "resource-vault.html", "Trainer Resource Vault", "resource-vault-root", "Track scarce resources that materially affect plans. Missing balances remain unknown, reserves win, and competing plans cannot silently spend the same budget twice.", vault_controls)
    _page(output_dir, "item-bag.html", "Item Bag Planner", "item-bag-root", "Set play-style targets, optional counts and reserves, review a free-slot scenario, and preserve rare or time-limited items. This planner never manipulates Pokémon GO.", bag_controls)
...
__all__ = ["LAB_VERSION", "TRADE_VERSION", "RESOURCE_VERSION", "ITEM_BAG_VERSION", "RESOURCE_TYPES", "ITEM_BAG_CATEGORIES", "build_trade_contract", "build_resource_contract", "build_item_bag_contract", "schemas", "publish"]