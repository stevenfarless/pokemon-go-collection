"""Collection-aware Event Calendar implementation for issue #151."""
...
def _pvp_signal(record: Mapping[str, Any]) -> float:
    ...
    for league in ("great", "ultra", "little"):
        value = ((record.get("pvp") or {}).get(league) or {}).get("rank_percent")
...
def _owned_maps(records: Iterable[Mapping[str, Any]]) -> tuple[dict, dict]:
...
def _source(index: Mapping[str, Any], fact: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "provider": index.get("provider"),
        "authority": index.get("classification"),
        "source_reference": fact.get("source_reference") or index.get("source_reference"),
        "source_references": list(index.get("source_references") or []),
        "dataset_timestamp": index.get("dataset_timestamp"),
        "data_version": index.get("data_version"),
        "freshness": dict(index.get("freshness") or {}),
        "validity": dict(index.get("validity") or {}),
        "snapshot_path": index.get("path"),
    }
...
def _overlays(fact: Mapping[str, Any], by_dex: dict, gaps: Mapping[str, Any], reference: Mapping[str, Any], roster: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for dex in featured:
        ...
        if gap.get("species_state") == "missing":
            missing.append({"dex": dex, "name": gap.get("name"), "route": (gap.get("links") or {}).get("reference") or f"reference.html?dex={dex}"})
    return {
...
def _restrictions(fact: Mapping[str, Any]) -> dict[str, Any]:
    ...
    details = {key: value for key, value in fact.items() if value not in (None, "", [], {}) and any(token in str(key).casefold() for token in tokens)}
    return {"state": "qualified" if details else "none-explicitly-published", "details": details}


def _evolution_deadlines(fact: Mapping[str, Any], source: Mapping[str, Any], by_name: dict, event_id: str, start: str, now: datetime) -> list[dict[str, Any]]:
    result: list[dict[str, Any]] = []
    for index, target in enumerate(fact.get("evolution_targets") or []):
        ...
        result.append({
            ...
            "exact_owned_records": [_record_summary(record) for record in by_name.get(_normal(source_name), [])],
...
def _exclusive_deadlines(fact: Mapping[str, Any], source: Mapping[str, Any], by_dex: dict, event_id: str, start: str, now: datetime) -> list[dict[str, Any]]:
    result: list[dict[str, Any]] = []
    for index, window in enumerate(fact.get("exclusive_windows") or []):
        ...
        frustration = "frustration" in f"{kind} {title}".casefold()
        exact = []
        if frustration:
            exact = [_record_summary(record) for dex in _event_dexes(fact) for record in by_dex.get(dex, []) if str((record.get("status") or {}).get("shadow_purified") or "").casefold() == "shadow"]
        result.append({
...
def build_calendar(output_dir: Path, manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for wrapped in _event_snapshots(output_dir):
        index = wrapped["index"]
        snapshot_states.append({"provider": index.get("provider"), "dataset_timestamp": index.get("dataset_timestamp"), "freshness": dict(index.get("freshness") or {}), "validity": dict(index.get("validity") or {}), "source_reference": index.get("source_reference"), "path": index.get("path")})
    ...
    return {
        ...
        "current_policy": {"required_snapshot_state": "fresh", "runtime_recheck": True, "stale_or_expired": "history-only; never shown in Now/Today/Next 7 days/Later actionable scopes", "unknown_time": "not actionable"},
        "history_policy": "Expired or stale reviewed events remain visible only in History with original evidence and timestamps.",
        "local_state": {"schema_version": LOCAL_STATE_VERSION, "storage": "browser-local-only", "contents": ["user reminders", "checklist completion", "selected scope"], "unified_backup": True, "canonical_collection_mutation": False},
        "snapshot_states": snapshot_states,
        "events": events,
        "deadlines": deadlines,
        "actionable_at_build_count": sum(1 for item in [*events, *deadlines] if item.get("actionable_at_build")),
        "collection_overlay_policy": {"exact_owned_record_ids": True, "missing_featured_species_source": "data/gap-radar.json", "weak_roster_types_source": "data/roster-readiness.json", "pvp_strength_signal": "Poke Genie rank_percent >= 98 only; not a current-meta ranking", "local_goals": "matched at runtime from browser-local goal state"},
    }
...
def schema() -> dict[str, Any]:
    return {
        ...
        "properties": {"schema_version": {"const": CALENDAR_VERSION}, "build_id": {"type": "string", "pattern": "^[0-9a-f]{12}$"}, "current_policy": {"type": "object"}, "history_policy": {"type": "string"}, "local_state": {"type": "object"}, "snapshot_states": {"type": "array"}, "events": {"type": "array"}, "deadlines": {"type": "array"}},
        "additionalProperties": True,
    }


def _page(output_dir: Path) -> None:
    html = '''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="Collection-aware Pokémon GO event agenda with freshness-gated deadlines and local reminders."><title>Event Calendar</title><link rel="stylesheet" href="assets/event-calendar.css" data-event-calendar-style></head><body><a class="skip-link" href="#event-calendar">Skip to content</a><main id="event-calendar" class="event-calendar-page"><header class="event-calendar-header ds-card"><p><a href="today.html">Today</a> · <a href="index.html">Collection</a> · <a href="tools.html">Tools</a> · <a href="opportunity-finder.html">Opportunities</a></p><p class="event-calendar-eyebrow">#151 · Collection-aware planning</p><h1>Event Calendar</h1><p>Chronological Pokémon GO deadlines with exact local time, source freshness, collection overlays, and browser-local reminders.</p><p id="event-calendar-timezone" class="event-calendar-note"></p></header><section class="event-calendar-controls ds-card" aria-labelledby="calendar-scope-heading"><h2 id="calendar-scope-heading">Agenda scope</h2><div class="event-calendar-scopes" role="group" aria-label="Event agenda scope"><button type="button" data-calendar-scope="now" aria-pressed="true">Now</button><button type="button" data-calendar-scope="today" aria-pressed="false">Today</button><button type="button" data-calendar-scope="next7" aria-pressed="false">Next 7 days</button><button type="button" data-calendar-scope="later" aria-pressed="false">Later</button><button type="button" data-calendar-scope="history" aria-pressed="false">History</button></div><p id="event-calendar-status" role="status"></p></section><section class="event-calendar-reminders ds-card" aria-labelledby="event-reminders-heading"><h2 id="event-reminders-heading">Local reminders</h2><p class="event-calendar-note">Stored only in this browser and included in unified local-data backup.</p><div class="event-calendar-reminder-form"><label>Reminder <input id="event-reminder-title" type="text" maxlength="120" autocomplete="off"></label><label>When <input id="event-reminder-at" type="datetime-local"></label><button id="event-reminder-add" type="button">Add reminder</button></div><div id="event-reminder-list"></div></section><div id="event-calendar-root" aria-live="polite"><p class="ds-empty">Loading event agenda…</p></div></main><script defer src="assets/event-calendar.js" data-event-calendar-script></script></body></html>'''
