...
(function exposePwaLifecycle(root, factory) {
  ...
  function createStatusUi(documentObject) {
    ...
    panel.id = panel.className = "pwa-lifecycle-status";
    panel.hidden = true;
    panel.setAttribute("role", "status"); panel.setAttribute("aria-live", "polite");
    const message = documentObject.createElement("span"); message.dataset.pwaMessage = "true";
    const install = documentObject.createElement("button"); install.type = "button"; install.dataset.pwaInstall = "true"; install.textContent = "Install"; install.hidden = true;
    const update = documentObject.createElement("button"); update.type = "button"; update.dataset.pwaUpdate = "true"; update.textContent = "Apply update"; update.hidden = true;
    const reload = documentObject.createElement("button"); reload.type = "button"; reload.dataset.pwaReload = "true"; reload.textContent = "Reload"; reload.hidden = true;
    panel.append(message, install, update, reload); documentObject.body.append(panel);
    return panel;
  }
  ...
  function install(root) {
    const documentObject = root.document, panel = createStatusUi(documentObject);
    const message = panel.querySelector("[data-pwa-message]"), installButton = panel.querySelector("[data-pwa-install]"), updateButton = panel.querySelector("[data-pwa-update]"), reloadButton = panel.querySelector("[data-pwa-reload]");
    const dirty = new Set();
    let deferredInstall = null, registration = null, waitingWorker = null;
    const show = (text) => { if (message) message.textContent = String(text || ""); panel.hidden = false; };
    const dirtyCount = () => dirty.size, markClean = (section) => { if (section) dirty.delete(section); };
    documentObject.addEventListener("input", (event) => { const section = editableSection(event.target); if (section) dirty.add(section); }, true);
    documentObject.addEventListener("change", (event) => { const section = editableSection(event.target); if (section && event.target?.type !== "file") dirty.add(section); }, true);
    documentObject.addEventListener("click", (event) => {
      const shareButton = event.target?.closest?.("#copy-link");
      if (shareButton && typeof root.navigator?.share === "function") {
        event.preventDefault(); event.stopImmediatePropagation();
        void share(root, { title: documentObject.title, url: root.location.href }); return;
      }
    ...
    const exposeWaiting = (worker) => {
      waitingWorker = worker; updateButton.hidden = false;
      show("A newer collection build is ready. Apply it when local edits are saved.");
    };
    updateButton.addEventListener("click", () => {
      if (dirtyCount()) { show(`Save or finish ${dirtyCount()} local edit area${dirtyCount() === 1 ? "" : "s"} before applying the update.`); return; }
    ...
    root.navigator.serviceWorker.addEventListener("controllerchange", () => {
      updateButton.hidden = true; reloadButton.hidden = false;
